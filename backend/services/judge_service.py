...
def get_judge_profile(db: Session, judge_id: int) -> dict | None:
    ...
    # Decision type distribution (result_type)
    ...
    return {
        ...
        "case_type_distribution": [{"type": r.trial_type or "未分類", "count": r.count} for r in case_type_dist],
        "result_type_distribution": [{"type": r.result_type or "未分類", "count": r.count} for r in decision_type_dist],
        "yearly_distribution": [{"year": r.year, "count": r.count} for r in yearly_dist],
        "role_distribution": [{"role": r.role or "未分類", "count": r.count} for r in role_dist],
        "courts_served": [{"court": r.court_name or "未分類", "count": r.count} for r in courts],
    }
...
def get_judge_cases(
    ...
    if case_type:
        count_query = count_query.where(Case.trial_type == case_type)
    if year:
